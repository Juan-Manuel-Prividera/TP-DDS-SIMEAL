  ...
  public Colaboracion crearColaboracion(TipoColaboracion tipoColaboracion, LocalDate fecha,
    ...
    return switch (tipoColaboracion) {
      case DINERO -> DonarDinero.builder()
          .cantidadDinero(cantidad).colaborador(colaborador).fechaDeRealizacion(fecha).build();
      case DONACION_VIANDA -> DonarVianda.builder().colaborador(colaborador)
          .fechaDeRealizacion(fecha).build();
      ...
      case ENTREGA_TARJETA -> DarDeAltaPersonaVulnerable.builder()
          .colaborador(colaborador).fechaDeRealizacion(fecha).build();
